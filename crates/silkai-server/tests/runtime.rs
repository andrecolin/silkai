...
async fn prefetch_then_soap_wakes_fake_engine() {
    ...
    let (job, mut tokens) = rt
        .submit_chat("soap", user("note"), Default::default())
        .await
        .unwrap();
...
async fn unknown_model_errors() {
    let rt = Runtime::new(clinic_cfg()).await.unwrap();
    let err = rt
        .submit_chat("nope", user("x"), Default::default())
        .await
        .unwrap_err();
...
async fn disabled_model_errors() {
    ...
    let err = rt
        .submit_chat("too-big", user("x"), Default::default())
        .await
        .unwrap_err();
...
async fn llama_cpp_submit_unavailable_without_feature() {
    ...
    let err = rt
        .submit_chat("soap", user("x"), Default::default())
        .await
        .unwrap_err();
    assert!(
...
async fn collect_chat(rt: &Runtime, model: &str, prompt: &str) -> String {
    let (job, mut tokens) = rt
        .submit_chat(model, user(prompt), Default::default())
        .await
        .unwrap();
    let mut out = String::new();
    while let Some(t) = tokens.recv().await {
...
async fn load_failure_unloads_and_next_submit_retries() {
    ...
    let err = rt
        .submit_chat("crashy-load", user("x"), Default::default())
        .await
        .unwrap_err();
    assert!(
...
async fn run_failure_unloads_and_next_submit_retries() {
    ...
    let err = rt
        .submit_chat("crashy-run", user("x"), Default::default())
        .await
        .unwrap_err();
    assert!(
        matches!(err, RuntimeError::Engine(_)),
...
async fn websocket_session_holds_slot_until_end() {
    ...
    let mut rx = rt
        .session_prompt(job, "whisper", &user("hello"), &Default::default())
        .await
        .unwrap();
    let mut out = String::new();
